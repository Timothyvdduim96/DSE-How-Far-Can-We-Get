...
####Take-off#####
theta_land=25*deg_to_rad
alpha_land=13*deg_to_rad
...
###Weight Distribution####
W_A=W_wing+W_eng+W_fuel_wing_landing
W_B=W_tail
W_rest=W_landing-W_A-W_B
W_array=[W_landing,W_A,W_B]
...
n=2.5*1.25
x_MG=21.43
######Reaction Force Main Gear##############
V_M=W_landing*g*(n-1)
def Fuselage_Shear_z(x):
    if 0<x<x_cg_wing:
        V=-w_rest*g*n*x
        
    elif x_cg_wing<x<x_MG:
         V=-w_rest*g*n*x+V_M
        
    elif x_MG<x<x_cg_tail:
        V=-w_rest*g*n*x+(L_wing-W_A*g*n)+V_M
        
    else :
        V=-w_rest*x+(L_wing-W_A*g*n)+(L_tail-W_B*g*n)+V_M
    return V
...
def Fuselage_Moment_z(x):
    if 0<x<x_cg_wing:
        ...
        M=-w_rest*g*n*x+(L_wing-W_A*g*n)+V_M*(x-x_MG)
        
    elif x_MG<x<x_cg_tail:
        M=-w_rest*g*n*x*2/2+(L_wing-W_A*g*n)*(x-x_cg_wing)+M_w+M_T+V_M*(x-x_MG)
    else :
        M=-w_rest*g*n*x*2/2+(L_wing-W_A*g*n)*(x-x_cg_wing)+M_w+M_T+(L_tail-W_B*g*n)*(x-x_cg_tail)++V_M*(x-x_MG)
    return M
