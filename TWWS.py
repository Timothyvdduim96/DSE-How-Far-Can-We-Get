...
if eval(ch) == 7:
    ...
    def cruise_limit(rho,C_L_clean_1,C_L_clean_2,C_L_clean_3,V_s):

        WS_1 = 0.5*rho*C_L_clean_1*V_s**2
        WS_2 = 0.5*rho*C_L_clean_2*V_s**2
        WS_3 = 0.5*rho*C_L_clean_3*V_s**2

        return WS_1,WS_2,WS_3

    #climb gradient limit

    def climb_gradient(cV,eff,C_D_0,A,e):

        C_D = 2*C_D_0
        C_L_1 = sqrt(C_D_0*pi*A*e)
        C_L_2 = sqrt(C_D_0*pi*A_2*e)
        C_L_3 = sqrt(C_D_0*pi*A_3*e)
        ...
        for i in range(len(WS)):
            ...
            WT_1.append(eff/(sqrt(WS[i])*(cV+C_D/C_L_1)*sqrt(2/(rho*C_L_1))))
            WT_2.append(eff/(sqrt(WS[i])*(cV+C_D/C_L_2)*sqrt(2/(rho*C_L_2))))
            WT_3.append(eff/(sqrt(WS[i])*(cV+C_D/C_L_3)*sqrt(2/(rho*C_L_3))))

        return WT_1,WT_2,WT_3

    #climb rate limit

    def climb_rate(c,rho,eff,C_D_0,A,A_2,A_3,e,xsize):
        ...
        for i in range(len(WS)):
            ...
            WT_1.append(eff/(c+sqrt(2*WS[i]/rho)/(1.345*(A*e)**0.75/C_D_0**0.25)))
            WT_2.append(eff/(c+sqrt(2*WS[i]/rho)/(1.345*(A_2*e)**0.75/C_D_0**0.25)))
            WT_3.append(eff/(c+sqrt(2*WS[i]/rho)/(1.345*(A_2*e)**0.75/C_D_0**0.25)))

        return WS,WT_1,WT_2,WT_3
    ...
    def take_off(TOP,C_L_TO_1,C_L_TO_2,C_L_TO_3,rho,rho_0):
        ...
        for i in range(len(WS)):
            ...
            WT_1.append(TOP*C_L_TO_1*sigma/WS[i])
            WT_2.append(TOP*C_L_TO_2*sigma/WS[i])
            WT_3.append(TOP*C_L_TO_3*sigma/WS[i])

        return WT_1,WT_2,WT_3

    #max speed limit

    def speed_limit(C_D_0,eff,rho_cruise,V_max,A,A_2,A_3,e):
        ...
        for i in range(len(WS)):
            ...
            WT_1.append(0.9/0.8*eff*(1.225/rho_cruise)**-0.75*((C_D_0*rho_cruise*0.5*V_max**3)/(0.8*WS[i]) + (0.8*WS[i])/(pi*A*e*0.5*rho_cruise*V_max))**-1)
            WT_2.append(0.9/0.8*eff*(1.225/rho_cruise)**-0.75*((C_D_0*rho_cruise*0.5*V_max**3)/(0.8*WS[i]) + (0.8*WS[i])/(pi*A_2*e*0.5*rho_cruise*V_max))**-1)
            WT_3.append(0.9/0.8*eff*(1.225/rho_cruise)**-0.75*((C_D_0*rho_cruise*0.5*V_max**3)/(0.8*WS[i]) + (0.8*WS[i])/(pi*A_3*e*0.5*rho_cruise*V_max))**-1)

        return WT_1,WT_2,WT_3

    def loadfactor_limit(C_D_0,eff,rho_cruise,V,n_max,A,A_2,A_3):

        n_max = n_max + 0.2 #safety factor
        TW_1 = []
        TW_2 = []
        TW_3 = []
        ...
        for i in range(len(WS)):
            ...
            TW_1.append(eff/(C_D_0*0.5*rho_cruise*V**3/WS[i]+WS[i]*n_max**2/(pi*A*e*0.5*rho_cruise*V)))
            TW_2.append(eff/(C_D_0*0.5*rho_cruise*V**3/WS[i]+WS[i]*n_max**2/(pi*A_2*e*0.5*rho_cruise*V)))
            TW_3.append(eff/(C_D_0*0.5*rho_cruise*V**3/WS[i]+WS[i]*n_max**2/(pi*A_3*e*0.5*rho_cruise*V)))
    ...
    plt.axvline(landlimit_1, linestyle="dashed", color="Red",label="Landing with CL=3.3")
    plt.axvline(landlimit_2, linestyle="dotted", color="Red",label="Landing with CL=3.5")
    plt.axvline(landlimit_3, linestyle="solid", color="Red",label="Landing with CL=3.7")
    ...
    plt.axvline(cruiselimit_1, linestyle="dashed", color="Blue",label="Cruising with CL=1.2")
    plt.axvline(cruiselimit_2, linestyle="dotted", color="Blue",label="Cruising with CL=1.5")
    plt.axvline(cruiselimit_3, linestyle="solid", color="Blue",label="Cruising with CL=1.8")

    xaxis = climb_rate(c,rho,eff,C_D_0,A,A_2,A_3,e,xsize)[0]

    climbgradientlimit_1 = climb_gradient(cV,eff,C_D_0,A,A_2,A_3,e)[0]
    climbgradientlimit_2 = climb_gradient(cV,eff,C_D_0,A,A_2,A_3,e)[1]
    climbgradientlimit_3 = climb_gradient(cV,eff,C_D_0,A,A_2,A_3,e)[2]
    plt.plot(xaxis,climbgradientlimit_1, linestyle="dashed", color="Green",label="Climb gradient with A=11")
    plt.plot(xaxis,climbgradientlimit_2, linestyle="dotted", color="Green",label="Climb gradient with A=12")
    plt.plot(xaxis,climbgradientlimit_3, linestyle="solid", color="Green",label="Climb gradient with A=13")

    climbratelimit_1 = climb_rate(c,rho,eff,C_D_0,A,A_2,A_3,e,xsize)[1]
    climbratelimit_2 = climb_rate(c,rho,eff,C_D_0,A,A_2,A_3,e,xsize)[2]
    climbratelimit_3 = climb_rate(c,rho,eff,C_D_0,A,A_2,A_3,e,xsize)[3]
    plt.plot(xaxis, climbratelimit_1, linestyle="dashed", color="Black",label="Climb rate with A=11")
    ...
    plt.plot(xaxis, takeofflimit_1, linestyle="dashed", color="Cyan",label="Taking off with CL=2.8")
    plt.plot(xaxis, takeofflimit_2, linestyle="dotted", color="Cyan",label="Taking off with CL=3.0")
    plt.plot(xaxis, takeofflimit_3, linestyle="solid", color="Cyan",label="Taking off with CL=3.2")

    maxspeedlimit_1 = speed_limit(C_D_0,eff,rho_cruise,V_max,A,A_2,A_3,e)[0]
    maxspeedlimit_2 = speed_limit(C_D_0,eff,rho_cruise,V_max,A,A_2,A_3,e)[1]
    maxspeedlimit_3 = speed_limit(C_D_0,eff,rho_cruise,V_max,A,A_2,A_3,e)[2]
    plt.plot(xaxis, maxspeedlimit_1, linestyle="dashed", color="Violet",label="Max speed with A=11")
    ...
    loadfactorlimit_1 = loadfactor_limit(C_D_0,eff,rho,V,n_max,A,A_2,A_3)[0]
    loadfactorlimit_2 = loadfactor_limit(C_D_0,eff,rho,V,n_max,A,A_2,A_3)[1]
    loadfactorlimit_3 = loadfactor_limit(C_D_0,eff,rho,V,n_max,A,A_2,A_3)[2]
    plt.plot(xaxis, loadfactorlimit_1, linestyle="dashed", color="Maroon",label="Load factor with A=11")
...
##            C_Lto = round(1.05*(to1-DPy)/(to1-to2)*C_L_TO_1+C_L_TO_1,2)
##        elif DPy < to2 and DPy >= to3:
##            C_Lto = round(1.05*(to2-DPy)/(to2-to3)*C_L_TO_2+C_L_TO_2,2)
##        else:
...
##        return C_Lto
    ...
    C_L_max_takeoff = raw_input("C_L_takeoff = ")
