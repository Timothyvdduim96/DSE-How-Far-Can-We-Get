...
CD = C_D_0 + (C_L_cr**2/(pi*A*e)) #use max clean?, Which value for CL, because drag polar only accurate up to a certain CL
#CD = 0.08 + (0.6**2/(pi*A*e))
D = CD * 0.5 * rho_0 * V**2 * S
L = C_L_cr * 0.5 * rho_0 * V**2 * S
