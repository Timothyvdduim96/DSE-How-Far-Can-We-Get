...
for i in range(len(x_cg_lst)):
    ...
    X_LEMAC = x_lemac_lst[i]*value("l_fus")#value("x_lemac")           #X-position leading edge mean aerodynamic chord
    cg_oew = x_cg_lst[i]#value("x_cg_oew")  #center of gravity at operational empty weight
    ...
    frontcg.append(min(min(xbarcg),min(xbarcgr)))
    aftcg.append(max(max(xbarcg),max(xbarcgr)))
...
plt.plot(frontcg,x_lemac_lst, color="black")
plt.plot(aftcg,x_lemac_lst, color="black")
